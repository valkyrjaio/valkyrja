...
return static function (Config $config): void {
    ...
    $srcRules[] = Rule::allClasses()
                      ->that(new ResideInOneOfTheseNamespaces('*'))
                      ->should(new NotContainDocBlockLike('* Class'))
                      ->because('All classes should NOT have redundant doc blocks');

    $srcRules[] = Rule::allClasses()
                      ->that(new IsInterface())
                      ->should(new NotContainDocBlockLike('* Contract'))
                      ->because('All contracts should NOT have redundant doc blocks');

    $srcRules[] = Rule::allClasses()
                      ->that(new IsInterface())
                      ->should(new NotContainDocBlockLike('* Interface'))
                      ->because('All contracts should NOT have redundant doc blocks');

    $srcRules[] = Rule::allClasses()
                      ->that(new IsTrait())
                      ->should(new NotContainDocBlockLike('* Trait'))
                      ->because('All traits should NOT have redundant doc blocks');

    $srcRules[] = Rule::allClasses()
                      ->that(new IsFinal())
                      ->should(new NotContainDocBlockLike('* Constant'))
                      ->because('All constants should NOT have redundant doc blocks');

    $srcRules[] = Rule::allClasses()
                      ->that(new IsEnum())
                      ->should(new NotContainDocBlockLike('* Enum'))
                      ->because('All enums should NOT have redundant doc blocks');

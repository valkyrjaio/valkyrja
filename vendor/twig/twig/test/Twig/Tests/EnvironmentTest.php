...
class Twig_Tests_EnvironmentTest extends PHPUnit_Valkyrja_TestCase
{
    ...
    public function testAutoescapeOption()
    {
        ...
        $this->assertEquals('foo\x3Cbr\x2F\x20\x3E foo\x3Cbr\x2F\x20\x3E', $twig->render('js', ['bar' => 'foo<br/ >']));
    }
    ...
    public function testGlobals()
    {
        ...
        $twig = new Twig_Environment($loader = new Twig_Loader_Array(['index' => '{{foo}}']));
        $twig->addGlobal('foo', 'foo');
        $twig->getGlobals();
        $twig->getFunctions();
        $twig->loadTemplate('index');
    ...
    public function testExtensionsAreNotInitializedWhenRenderingACompiledTemplate()
    {
        ...
        $twig = $this->getMockBuilder('Twig_Environment')->setConstructorArgs(
                [
                    $loader,
                    $options,
                ]
            )->setMethods(['initExtensions'])->getMock();
    ...
    public function testAutoReloadCacheMiss()
    {
        ...
        $cache->expects($this->once())->method('getTimestamp')->will($this->returnValue(0));
        $loader->expects($this->never())->method('isFresh');
    ...
    public function testAutoReloadCacheHit()
    {
        ...
        $loader->expects($this->once())->method('isFresh')->will($this->returnValue(true));
        $cache->expects($this->once())->method('load');
    ...
    public function testAddExtensionWithDeprecatedGetGlobals()
    {
        ...
        $this->assertContains(
            'Defining the getGlobals() method in the "environment_test" extension ',
            $this->deprecations[0]
        );
    ...
    public function testInitRuntimeWithAnExtensionUsingInitRuntimeDeprecation()
    {
        ...
        $this->assertCount(1, $this->deprecations);
        $this->assertContains(
            'Defining the initRuntime() method in the "with_deprecation" extension is deprecated since version 1.23.',
            $this->deprecations[0]
        );
    ...
    protected function getMockLoader($templateName, $templateContent)
    {
        ...
        $loader->expects($this->any())->method('getSource')->with($templateName)->will(
                $this->returnValue($templateContent)
            );
        $loader->expects($this->any())->method('getCacheKey')->with($templateName)->will(
                $this->returnValue($templateName)
            );

...
class Twig_Tests_ErrorTest extends PHPUnit_Valkyrja_TestCase
{
    public function testErrorWithObjectFilename()
    {
        ...
        $this->assertContains(
            'test' . DIRECTORY_SEPARATOR . 'Twig' . DIRECTORY_SEPARATOR . 'Tests' . DIRECTORY_SEPARATOR . 'ErrorTest.php',
            $error->getMessage()
        );
    }
    ...
    public function testTwigExceptionAddsFileAndLineWhenMissingWithInheritanceOnDisk()
    {
        ...
        try {
            ...
            $this->assertEquals(
                'An exception has been thrown during the rendering of a template ("Runtime error...") in "index.html" at line 3.',
                $e->getMessage()
            );
            $this->assertEquals(3, $e->getTemplateLine());
            $this->assertEquals('index.html', $e->getTemplateFile());
    ...
    public function testTwigExceptionAddsFileAndLine($templates, $name, $line)
    {
        ...
        try {
            ...
            $this->assertEquals(
                sprintf('Variable "foo" does not exist in "%s" at line %d', $name, $line),
                $e->getMessage()
            );

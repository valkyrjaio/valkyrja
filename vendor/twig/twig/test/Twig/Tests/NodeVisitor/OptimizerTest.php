...
class Twig_Tests_NodeVisitor_OptimizerTest extends PHPUnit_Valkyrja_TestCase
{
    ...
    public function testRenderParentBlockOptimizer()
    {
        ...
        $stream = $env->parse(
            $env->tokenize('{% extends "foo" %}{% block content %}{{ parent() }}{% endblock %}', 'index')
        );
    ...
    public function testForOptimizer($template, $expected)
    {
        ...
        foreach ($expected as $target => $withLoop) {
            $this->assertTrue(
                $this->checkForConfiguration($stream, $target, $withLoop),
                sprintf('variable %s is %soptimized', $target, $withLoop ? 'not ' : '')
            );
        }
    }

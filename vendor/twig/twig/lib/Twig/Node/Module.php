...
class Twig_Node_Module extends Twig_Node
{
    ...
    protected function compileTemplate(Twig_Compiler $compiler)
    {
        ...
        if (count($this->getNode('blocks')) || count($this->getNode('traits')) || !$this->hasNode(
                'parent'
            ) || $this->getNode('parent') instanceof Twig_Node_Expression_Constant || count(
    ...
    protected function compileGetParent(Twig_Compiler $compiler)
    {
        ...
        if ($parent instanceof Twig_Node_Expression_Constant) {
            ...
            $compiler->raw('$this->loadTemplate(')->subcompile($parent)->raw(', ')->repr(
                    $this->getAttribute('filename')
                )->raw(', ')->repr($parent->getLine())->raw(')');
        }
    ...
    protected function compileConstructor(Twig_Compiler $compiler)
    {
        ...
        elseif (($parent = $this->getNode('parent')) && $parent instanceof Twig_Node_Expression_Constant) {
            $compiler->addDebugInfo($parent)->write('$this->parent = $this->loadTemplate(')->subcompile($parent)->raw(
                    ', '
                )->repr($this->getAttribute('filename'))->raw(', ')->repr($parent->getLine())->raw(");\n");
        }
        ...
        if ($countTraits) {
            ...
            foreach ($this->getNode('traits') as $i => $trait) {
                ...
                $compiler->addDebugInfo($trait->getNode('template'))->write(
                        sprintf("if (!\$_trait_%s->isTraitable()) {\n", $i)
                    )->indent()->write("throw new Twig_Error_Runtime('Template \"'.")->subcompile(
                        $trait->getNode('template')
                    )->raw(".'\" cannot be used as a trait.');\n")->outdent()->write("}\n")->write(
                        sprintf("\$_trait_%s_blocks = \$_trait_%s->getBlocks();\n\n", $i, $i)
                    );

                foreach ($trait->getNode('targets') as $key => $value) {
                    $compiler->write(sprintf('if (!isset($_trait_%s_blocks[', $i))->string($key)->raw(
                            "])) {\n"
                        )->indent()->write("throw new Twig_Error_Runtime(sprintf('Block ")->string($key)->raw(
                            ' is not defined in trait '
                        )->subcompile($trait->getNode('template'))->raw(".'));\n")->outdent()->write("}\n\n")->write(
                            sprintf('$_trait_%s_blocks[', $i)
                        )->subcompile($value)->raw(sprintf('] = $_trait_%s_blocks[', $i))->string($key)->raw(
                            sprintf(']; unset($_trait_%s_blocks[', $i)
                        )->string($key)->raw("]);\n\n");
                }
            }
    ...
    {
        $compiler->write("public function getSource()\n", "{\n")->indent()->write('return ')->string(
    ...
    protected function compileLoadTemplate(Twig_Compiler $compiler, $node, $var)
    {
        if ($node instanceof Twig_Node_Expression_Constant) {
            $compiler->write(sprintf('%s = $this->loadTemplate(', $var))->subcompile($node)->raw(', ')->repr(
                    $this->getAttribute('filename')
                )->raw(', ')->repr($node->getLine())->raw(");\n");
        }
        else {

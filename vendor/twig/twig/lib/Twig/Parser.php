...
class Twig_Parser implements Twig_ParserInterface
{
    ...
    public function setMacro($name, Twig_Node_Macro $node)
    {
        if ($this->isReservedMacroName($name)) {
            throw new Twig_Error_Syntax(
                sprintf('"%s" cannot be used as a macro name as it is a reserved keyword.', $name),
                $node->getLine(),
                $this->getFilename()
            );
        }
    ...
    protected function filterBodyNodes(Twig_NodeInterface $node)
    {
        ...
        if (($node instanceof Twig_Node_Text && !ctype_space(
                    $node->getAttribute('data')
                )) || (!$node instanceof Twig_Node_Text && !$node instanceof Twig_Node_BlockReference && $node instanceof Twig_NodeOutputInterface)
        ) {
            if (false !== strpos((string)$node, chr(0xEF) . chr(0xBB) . chr(0xBF))) {
                throw new Twig_Error_Syntax(
                    'A template that extends another one cannot have a body but a byte order mark (BOM) has been detected; it must be removed.',
                    $node->getLine(),
                    $this->getFilename()
                );
            }

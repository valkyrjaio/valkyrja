...
class Twig_ExpressionParser
{
    ...
    public function parseHashExpression()
    {
        ...
        while (!$stream->test(Twig_Token::PUNCTUATION_TYPE, '}')) {
            ...
            if (($token = $stream->nextIf(Twig_Token::STRING_TYPE)) || ($token = $stream->nextIf(
                    Twig_Token::NAME_TYPE
                )) || $token = $stream->nextIf(Twig_Token::NUMBER_TYPE)
            ) {
            ...
            elseif ($stream->test(Twig_Token::PUNCTUATION_TYPE, '(')) {
                ...
                throw new Twig_Error_Syntax(
                    sprintf(
                        ...
                        $current->getValue()
                    ), $current->getLine(), $this->parser->getFilename()
                );
            }
    ...
    public function parseSubscriptExpression($node)
    {
        ...
        if ($token->getValue() == '.') {
            $token = $stream->next();
            if ($token->getType() == Twig_Token::NAME_TYPE || $token->getType(
                ) == Twig_Token::NUMBER_TYPE || ($token->getType() == Twig_Token::OPERATOR_TYPE && preg_match(
            ...
            if ($node instanceof Twig_Node_Expression_Name && null !== $this->parser->getImportedSymbol(
                    'template',
                    $node->getAttribute(
                        'name'
                    )
                ...
                if ($this->parser->isReservedMacroName($name)) {
                    throw new Twig_Error_Syntax(
                        sprintf('"%s" cannot be called as macro as it is a reserved keyword.', $name),
                        $token->getLine(),
                        $this->parser->getFilename()
                    );
                }
            ...
            if ($slice) {
                ...
                $filter = new $class(
                    $node,
                    new Twig_Node_Expression_Constant('slice', $token->getLine()),
                    $arguments,
                    $token->getLine()
                );

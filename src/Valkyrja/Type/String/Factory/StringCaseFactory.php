...
class StringCaseFactory
{
    ...
    {
        return static::toCapitalized(StringFactory::replaceAllWith($subject, ['-', '_'], ' '), $delimiter);
    }
    ...
    public static function toStudlyCase(string $subject): string
    {
        return self::$studlyCache[$subject]
            ?? (self::$studlyCache[$subject] = StringFactory::replace(static::toCapitalizedWords($subject), ' ', ''));
    }
    ...
    {
        return static::toUpperCase(StringFactory::substr($subject, 0, 1)) . StringFactory::substr($subject, 1);
    }

...
carries `ExitCode::ERROR`, so a `--quiet` run leaves each one alone. A plain
report builds its own `Output` instead, which no configured factory can
redirect and no flag suppresses.

`InputHandler::handle()` and `InputHandler::run()` both build a plain report.
...
Every plain report names the command. It names none when reading the command
...
plain report, which echoes. The code the run's own output reports at that
...
the first report raises. The plain report then echoes. Register a
...
a plain report then, which names the throwable it answered and the

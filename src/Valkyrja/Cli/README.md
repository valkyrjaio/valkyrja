...
interaction flags govern that report. A second report takes a plain `Output`
instead. That report carries the default flags, so it echoes on a `--quiet` or
`--silent` run, and no configured factory can redirect it.

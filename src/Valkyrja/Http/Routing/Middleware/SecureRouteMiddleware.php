 ...
 */
class SecureRouteMiddleware implements RouteMatchedMiddleware
{
    ...
    public function routeMatched(ServerRequest $request, Route $route, RouteMatchedHandler $handler): Route|Response
    {
        ...
        if (! $request->getUri()->isSecure()) {
            // Redirect to the secure path
            return $this->responseFactory
                ->createRedirectResponse()
                ->secure($request->getUri()->getPath(), $request);
        }
